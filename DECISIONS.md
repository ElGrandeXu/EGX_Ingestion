...
| 2026-06-18 | EXP-0002 | Experiment | Promote `minimal-implementation-ladder` to active EGX_Ingestion rule | Bounded next-action task was improved with a small Markdown/template patch while avoiding scripts, tickets, folders, dependencies and plugin activation | Adds short AGENTS pointer, theme doctrine and reusable next-mission template | Use on next source or implementation experiment | Active |
| 2026-06-18 | EXP-0002 | Production candidate | Keep PC-0001 as candidate, not accepted production doctrine | Experiment validates ingestion-workspace usefulness but not a real production implementation task | EGX_Production impact remains candidate principle | Test on one production-style implementation task later | Active |
